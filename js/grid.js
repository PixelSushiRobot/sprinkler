...
export function renderGrid() {
  ...
  order.forEach((origIdx, pos) => {
    ...
    t.innerHTML = `<div class="tilefill" style="height:${fillH}%"></div>${crownHtml}<button class="x" title="remove" aria-label="remove ${escapeHTML(c.name)}">×</button><img class="av" src="${escapeHTML(c.avatar || avatarSVG(c.id))}" alt=""><div class="nm">${escapeHTML(c.name)}</div>${statusHtml}${numHtml}`;
    t.dataset.id = c.id;
